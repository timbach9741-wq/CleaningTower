...
const qnaData = {
  ...
  "5. 가입 및 정보 등록 관련 질문 & 답변": [
    ...
    ["Q11. 가입 시 인증번호 문자가 안 오거나 알림 관련 오류가 납니다. 어떻게 하나요?", "통신사 스팸 차단 설정이나 일시적 트래픽 지연으로 문자가 오지 않을 수 있습니다. 휴대폰 번호를 숫자만 올바르게 입력했는지 재확인하고, 스팸 차단 앱이나 스팸 메세지함을 확인해 주세요. 지속적으로 지연되는 경우 본사 고객센터(031-499-9509) 또는 우측 하단 카카오톡 상담 채널로 성함과 연락처를 남겨주시면 본사에서 즉시 직접 인증 및 가입 승인 처리를 도와드립니다."],
    ["Q12. 사진 업로드(프로필, 포트폴리오 등) 시 오류가 발생합니다. 어떻게 하나요?", "사진 용량이 너무 크거나(10MB 초과) 허용되지 않는 파일 형식일 때 오류가 발생할 수 있습니다. 이미지 용량을 낮추어 다시 시도하거나, 카카오톡 인앱 브라우저의 파일 첨부 제한 때문일 수 있으니 우측 상단의 메뉴 버튼을 눌러 [다른 브라우저로 열기](Chrome, Safari 등)를 통해 재접속해 보세요. 문제가 해결되지 않는 경우, 등록하실 사진을 카카오톡 채널로 전송해 주시면 본사에서 즉시 대신 등록 처리해 드립니다."]
  ],
